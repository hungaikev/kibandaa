...
import (
	...
	"github.com/hungaikev/kibandaa/payments/internal/api/handlers"
...
func main() {
	...
	z := zerolog.New(os.Stderr).With().Str(LogStrKeyService, "payments").Timestamp().Logger().With().Caller().Logger()
...
func run(log *zerolog.Logger) error {
	...
	paymentsServer := api.NewPaymentsServer(log, build)

	server := handlers.API(paymentsServer, cfg.Web.APIPort)
